...
class Server
{
    ...
    public function __construct($port = self::DEFAULT_PORT, $address = self::DEFAULT_ADDRESS)
    {
    ...
    private function _createSocket()
    {
        ...
        if ($this->sockServer === false) {
            throw new Exception(Exception::CANT_CREATE_SOCKET, socket_strerror(socket_last_error()));
        }
    ...
    private function _bindSocket()
    {
        if (socket_bind($this->sockServer, $this->address, $this->port) === false) {
            throw new Exception(Exception::CANT_BIND_SOCKET, socket_strerror(socket_last_error($this->sockServer)));
        }
    }
    ...
    protected function serverLoop()
    {
        while ($this->_listenLoop) {
            if (($socket = @socket_accept($this->sockServer)) === false) {
                throw new Exception(Exception::CANT_ACCEPT, socket_strerror(socket_last_error($this->sockServer)));
                continue;
            }
